...
@Transactional //이 어노테이션을 선언합니다. 로직을 처리하다가
                //에러가 발생했다면 변경된 데이터로직을 수행하기
                //이전 상태로 롤백 시켜줍니다.
@RequiredArgsConstructor
public class MemberService implements UserDetailsService {

    private final MemberRepository memberRepository;
    //빈 주입하는 방법으로 @Autowired 어노테이션을 사용하거나 필드 주입
    //setter를 사용하거나 생성자를 새로 만들어서 주입하는 방법 등이 있다.
    //@RequiredArgsConstructor 어노테이션은 final이나
    //@NonNull 이 붙은 필드에 생성자를 생성해 줌. 빈이라서 new~~이거 생성자
    ...
    public UserDetails loadUserByUsername(String email) throws UsernameNotFoundException {
        ...
        return User.builder()
                .username(member.getEmail())    //세션에 저장 됨
    ...
    //회원가입시 회원 가입 여부 확인 하는 메소드
    private void validateDuplicateMember(String email){

        Member member =
        memberRepository.findByEmail(email);
        //member이 null이라는건 db에 회원가입이
        //되어있지 않은 email이니 회원가입이 가능하고
        //null이 아니라는건 db에 회원이 가입되어있으니
        //회원가입을 막거나 예외처리 등을 수행하자
        if(member != null){
            throw new IllegalStateException("이미 가입된 회원입니다.");
        }
        // 이 내용은 try{} chach(IllegalStateException e) {
